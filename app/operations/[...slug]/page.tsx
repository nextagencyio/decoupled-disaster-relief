...
interface OperationByPathData {
  route: {
    entity: DrupalOperation
  } | null
}
...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  const path = `/operations/${slug.join('/')}`
  const item = await getOperation(path)

  if (!item) {
    return { title: 'Operation Not Found | Disaster Relief' }
  }
              ...
              <div className="bg-white rounded-xl shadow-sm p-6 sticky top-24">
                ...
                <dl className="space-y-4">
                  {(item as any).operationStatus && (
                    <div>
                      <dt className="text-sm text-gray-500">Operation Status</dt>
                      <dd className="font-semibold text-gray-900">{(item as any).operationStatus}</dd>
                    </div>
                  )}
                  ...
                  {(item as any).peopleServed && (
                    <div>
                      <dt className="text-sm text-gray-500">People Served</dt>
                      <dd className="font-semibold text-gray-900">{(item as any).peopleServed}</dd>
                    </div>
                  )}
                </dl>
                <div className="mt-8">
