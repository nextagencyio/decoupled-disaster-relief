...
import { GET_UPDATE_BY_PATH } from '@/lib/queries'
import { DrupalUpdate } from '@/lib/types'
...
async function getUpdate(path: string): Promise<DrupalUpdate | null> {
  try {
    ...
    const { data } = await apolloClient.query<UpdateByPathData>({
      query: GET_UPDATE_BY_PATH,
      variables: { path },
...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  const path = `/updates/${slug.join('/')}`
  const item = await getUpdate(path)

  if (!item) {
    return { title: 'Update Not Found | Disaster Relief' }
  }

  return {
    title: `${item.title} | Disaster Relief`,
            ...
            <div className="lg:col-span-1">
              <div className="bg-white rounded-xl shadow-sm p-6 sticky top-24">
                ...
                <dl className="space-y-4">
                  {(item as any).operationName && (
                    <div>
                      <dt className="text-sm text-gray-500">Operation Name</dt>
                      <dd className="font-semibold text-gray-900">{(item as any).operationName}</dd>
                    </div>
                  )}
                  {(item as any).urgencyLevel && (
                    <div>
                      <dt className="text-sm text-gray-500">Urgency Level</dt>
                      <dd className="font-semibold text-gray-900">{(item as any).urgencyLevel}</dd>
                    </div>
                  )}
                </dl>
                <div className="mt-8">
